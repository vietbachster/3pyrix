 ...
 *
 * XTC is the native binary ebook format used here for pre-rendered X3 pages.
...
// XTeink X3 portrait page resolution
constexpr uint16_t DISPLAY_WIDTH = 528;
constexpr uint16_t DISPLAY_HEIGHT = 792;
...
struct PageTableEntry {
  ...
  uint16_t width;       // 0x0C: Page width (528)
  uint16_t height;      // 0x0E: Page height (792)
};
#pragma pack(pop)
