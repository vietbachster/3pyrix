...
void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line) {
  ...
  const int lineHeight = config.getReadingLineHeight();
...
void ChapterHtmlSlimParser::makePages() {
  ...
  // Extra paragraph spacing based on spacingLevel (0=none, 1=small, 2=relaxed, 3=large)
  ...
  if (!stopRequested_) {
    switch (config.spacingLevel) {
      case 1:
        ...
        break;
      case 2:
        currentPageNextY += lineHeight / 2;  // Relaxed (1/2 line)
        break;
      case 3:
