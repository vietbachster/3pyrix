...
class GfxRenderer {
 public:
  ...
  enum Orientation {
    Portrait,                  // 528x792 logical coordinates (current default)
    LandscapeClockwise,        // 792x528 logical coordinates, rotated 180° (swap top/bottom)
    PortraitInverted,          // 528x792 logical coordinates, inverted
    LandscapeCounterClockwise  // 792x528 logical coordinates, native panel orientation
  };
 ...
 private:
  ...
  // Sized for the widest X3 dimension.
 ...
 public:
  ...
  void getTextBounds(int fontId, const char* text, int* minX, int* minY, int* maxX, int* maxY,
