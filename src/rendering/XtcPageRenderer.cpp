...
XtcPageRenderer::RenderResult XtcPageRenderer::render(xtc::XtcParser& parser, uint32_t pageNum,
  ...
  const bool isX3Portrait = pageWidth == xtc::DISPLAY_WIDTH && pageHeight == xtc::DISPLAY_HEIGHT;
  const bool isX3Landscape = pageWidth == xtc::DISPLAY_HEIGHT && pageHeight == xtc::DISPLAY_WIDTH;
  if (!isX3Portrait && !isX3Landscape) {
    LOG_ERR(TAG, "Unsupported XTC page size %ux%u for X3 build", pageWidth, pageHeight);
    return RenderResult::InvalidDimensions;
  }
