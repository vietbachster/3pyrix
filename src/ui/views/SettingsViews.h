...
struct ReaderSettingsView {
  ...
  static constexpr const char* const FONT_SIZE_VALUES[] = {"Small", "Large"};
  ...
  static constexpr const char* const AUTO_TURN_MODE_VALUES[] = {"OFF", "ON"};
  static constexpr const char* const AUTO_TURN_WPM_VALUES[] = {"Slow (450)", "Normal (490)", "Fast (495)", "V.Fast (500)", "Expert (505)"};
  ...
  char themeIds[MAX_THEMES][32] = {};
  char themeLabels[MAX_THEMES][32] = {};
  int themeCount = 0;
  ...
  const char* getCurrentValueStr(int index) const {
    ...
    if (def.type == SettingType::ThemeSelect) {
      if (themeCount > 0 && currentThemeIndex < themeCount) {
        return themeLabels[currentThemeIndex];
      }
      return "light";
    }
  ...
  const char* getCurrentThemeName() const {
    if (themeCount > 0 && currentThemeIndex < themeCount) {
      return themeIds[currentThemeIndex];
    }
    return "light";
  }
};
