...
// Version 12: Removed fixed auto page turn mode and speed setting
constexpr uint8_t MIN_SETTINGS_VERSION = 12;
constexpr uint8_t SETTINGS_FILE_VERSION = 12;
// Increment this when adding new persisted settings fields
constexpr uint8_t SETTINGS_COUNT = 28;

uint8_t normalizeReaderFontSize(uint8_t fontSize) {
  return fontSize == Settings::FontLarge ? Settings::FontLarge : Settings::FontSmall;
}
}  // namespace
...
Result<void> Settings::save(drivers::Storage& storage) const {
  ...
  serialization::writePod(outputFile, _reserved2);
  ...
  // Write string fields — track return values to detect disk-full truncation mid-write
  bool writeOk = true;
  writeOk &= (outputFile.write(reinterpret_cast<const uint8_t*>(themeName), sizeof(themeName)) == sizeof(themeName));
  writeOk &= (outputFile.write(reinterpret_cast<const uint8_t*>(lastBookPath), sizeof(lastBookPath)) == sizeof(lastBookPath));
  ...
  writeOk &= (outputFile.write(reinterpret_cast<const uint8_t*>(fileListDir), sizeof(fileListDir)) == sizeof(fileListDir));
  writeOk &= (outputFile.write(reinterpret_cast<const uint8_t*>(fileListSelectedName), sizeof(fileListSelectedName)) == sizeof(fileListSelectedName));
  serialization::writePod(outputFile, fileListSelectedIndex);
  serialization::writePod(outputFile, frontButtonLayout);
  serialization::writePod(outputFile, autoPageTurnMode);
  serialization::writePod(outputFile, autoPageTurnWpm);
  serialization::writePod(outputFile, tiltPageTurn);
  outputFile.close();

  if (!writeOk) {
    LOG_ERR(TAG, "Write error while saving settings (disk full?)");
...
Result<void> Settings::load(drivers::Storage& storage) {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPodValidated(inputFile, _reserved2, uint8_t(3));
    if (++settingsRead >= fileSettingsCount) break;
    ...
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPodValidated(inputFile, autoPageTurnMode, uint8_t(2));
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPodValidated(inputFile, autoPageTurnWpm, uint8_t(5));
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPodValidated(inputFile, tiltPageTurn, uint8_t(2));
    if (++settingsRead >= fileSettingsCount) break;
  } while (false);
...
int Settings::getReaderFontId(const Theme& theme) const {
  switch (normalizeReaderFontSize(fontSize)) {
    case FontLarge:
...
RenderConfig Settings::getRenderConfig(const Theme& theme, uint16_t viewportWidth, uint16_t viewportHeight) const {
  return RenderConfig(getReaderFontId(theme), getLineCompression(), normalizeReaderFontSize(fontSize), getIndentLevel(),
                      getSpacingLevel(), paragraphAlignment, static_cast<bool>(hyphenation),
                      static_cast<bool>(showImages), viewportWidth, viewportHeight);
}
