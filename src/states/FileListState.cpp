...
void normalizeDirectory(char* path, size_t pathSize, drivers::Storage& storage) {
  if (!path || pathSize == 0 || path[0] == '\0') {
    strncpy(path, "/", pathSize - 1);
    path[pathSize - 1] = '\0';
    return;
  }

  while (strcmp(path, "/") != 0 && !directoryExists(storage, path)) {
...
void FileListState::enter(Core& core) {
  ...
  const bool preservePosition = transition.isValid() && transition.returnTo == ReturnTo::FILE_MANAGER;
  ...
  normalizeDirectory(currentDir_, sizeof(currentDir_), core.storage);
  ...
  if (core.pendingSync == SyncMode::FileTransfer && core.networkReturnState == StateId::FileList) {
...
void FileListState::loadFiles(Core& core) {
  files_.clear();
  files_.reserve(64);  // Initial reservation; grows as needed
...
bool FileListState::isSupportedFile(const char* name) const {
  ...
  // Case-insensitive extension check; keep in sync with detectContentType().
        ...
        if (currentScreen_ == Screen::ConfirmDelete) {
            ...
            case Button::Center:
              if (confirmView_.isYesSelected()) {
                ...
                if (activeBook[0] != '\0' && strcmp(pathBuf, activeBook) == 0) {
                  ...
                  if (result.ok() && entry.isDir) {
                    normalizeDirectory(core.settings.fileListDir, sizeof(core.settings.fileListDir), core.storage);
                    // Persist the updated fileListDir so it survives sleep/restart
          ...
          switch (e.button) {
            ...
            case Button::Left:
              ...
              core.networkReturnState = StateId::FileList;
...
void FileListState::saveBrowsePosition(Core& core) {
  strncpy(core.settings.fileListDir, currentDir_, sizeof(core.settings.fileListDir) - 1);
  core.settings.fileListDir[sizeof(core.settings.fileListDir) - 1] = '\0';

  if (!files_.empty() && selectedIndex_ < files_.size()) {
    strncpy(core.settings.fileListSelectedName, files_[selectedIndex_].name.c_str(),
            sizeof(core.settings.fileListSelectedName) - 1);
