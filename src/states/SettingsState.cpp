...
namespace {
uint8_t settingsFontSizeToUiValue(uint8_t fontSize) {
  return fontSize == Settings::FontLarge ? 1 : 0;
}

uint8_t uiValueToSettingsFontSize(uint8_t value) {
  return value == 1 ? Settings::FontLarge : Settings::FontSmall;
}
}  // namespace
    ...
    case SettingsScreen::ConfirmDialog:
      if (confirmView_.isYesSelected()) {
        if (pendingAction_ == 10) {
          ...
          // Clear Device Storage - erase all app data on SD card
          ui::centeredMessage(renderer_, THEME, THEME.uiFontId, "Clearing device data...");
...
void SettingsState::loadReaderSettings() {
  ...
  for (size_t i = 0; i < themes.size() && i < ui::ReaderSettingsView::MAX_THEMES; i++) {
    strncpy(readerView_.themeIds[i], themes[i].c_str(), sizeof(readerView_.themeIds[i]) - 1);
    readerView_.themeIds[i][sizeof(readerView_.themeIds[i]) - 1] = '\0';

    const char* displayName = THEME_MANAGER.getThemeDisplayName(themes[i].c_str());
    strncpy(readerView_.themeLabels[i], displayName, sizeof(readerView_.themeLabels[i]) - 1);
    readerView_.themeLabels[i][sizeof(readerView_.themeLabels[i]) - 1] = '\0';
  ...
  // Index 1: Font Size
  readerView_.values[1] = settingsFontSizeToUiValue(settings.fontSize);

  // Index 2: Text Anti-Aliasing (toggle)
  readerView_.values[2] = settings.textAntiAliasing;

  // Index 3: Paragraph Alignment (0=Justified, 1=Left, 2=Center, 3=Right)
  readerView_.values[3] = settings.paragraphAlignment;

  // Index 4: Hyphenation (toggle)
  readerView_.values[4] = settings.hyphenation;

  // Index 5: Show Images (toggle)
  readerView_.values[5] = settings.showImages;

  // Index 6: Reading Orientation (0=Portrait, 1=Landscape CW, 2=Inverted, 3=Landscape CCW)
  readerView_.values[6] = settings.orientation;

  // Index 7: Tilt Page Turn (toggle)
  readerView_.values[7] = settings.tiltPageTurn;

  // Index 8: Auto Page Turn Mode (0=OFF, 1=ON)
  readerView_.values[8] = settings.autoPageTurnMode;

  // Index 9: Reading WPM preset for auto page turn when ON
  readerView_.values[9] = settings.autoPageTurnWpm;
}
...
void SettingsState::saveReaderSettings() {
  ...
  // Index 1: Font Size
  settings.fontSize = uiValueToSettingsFontSize(readerView_.values[1]);

  // Text Layout and Line Spacing are fixed at runtime.

  // Index 2: Text Anti-Aliasing
  settings.textAntiAliasing = readerView_.values[2];

  // Index 3: Paragraph Alignment
  ...
  // Index 6: Reading Orientation
  settings.orientation = readerView_.values[6];

  // Index 7: Tilt Page Turn
  settings.tiltPageTurn = readerView_.values[7];

  // Index 8: Auto Page Turn Mode
  settings.autoPageTurnMode = readerView_.values[8];

  // Index 9: Reading WPM
  settings.autoPageTurnWpm = readerView_.values[9];
}
...
void SettingsState::clearCache(int type, Core& core) {
  ...
  if (type == 0) {
    ...
    confirmView_.setup("Clear Device?", "This will erase all app data,", "settings and WiFi. Will restart.");
    pendingAction_ = 11;
    currentScreen_ = SettingsScreen::ConfirmDialog;
    needsRender_ = true;
    return;
