...
std::string findHomeCoverPath(ContentHandle& content) {
  std::string path = content.getThumbnailPath();
  if (!path.empty() && SdMan.exists(path.c_str())) {
    return path;
  }

  path = content.getCoverPath();
  if (!path.empty() && SdMan.exists(path.c_str())) {
    return path;
  }
...
void HomeState::loadLastBook(Core& core) {
  ...
  if (core.content.isOpen()) {
    ...
    // Home cover should not depend on the reader's inline-image setting.
    coverBmpPath_ = findHomeCoverPath(core.content);
    if (!coverBmpPath_.empty()) {
      hasCoverImage_ = true;
      LOG_DBG(TAG, "Using cached home cover: %s", coverBmpPath_.c_str());
    }
      ...
      case EventType::ButtonPress:
        switch (e.button) {
          ...
          case Button::Left:
            ...
            core.networkReturnState = StateId::Home;
...
void HomeState::render(Core& core) {
  ...
  // Home screen text is UI chrome, not reader content.
