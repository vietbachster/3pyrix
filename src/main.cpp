...
#define SPI_FQ 10000000
// Display SPI pins for XTEINK X3
...
#define SERIAL_READY_TIMEOUT_MS 50
...
EpdFontFamily uiFontFamily(&ui12Font);
...
// In READER mode this saves memory by not instantiating unused sizes.
...
void verifyWakeupLongPress(esp_reset_reason_t resetReason) {
  ...
  // X3 wake handling is more reliable via direct GPIO read than via the ADC/button
  // scan path during the first few hundred milliseconds after boot.
  while (digitalRead(InputManager::POWER_BUTTON_PIN) != LOW && millis() - start < 1000) {
    delay(10);
  }
  ...
  if (digitalRead(InputManager::POWER_BUTTON_PIN) == LOW) {
    ...
    do {
      delay(5);
    } while (digitalRead(InputManager::POWER_BUTTON_PIN) == LOW && millis() - holdStart < requiredPressDuration);
    abort = millis() - holdStart < requiredPressDuration;
  } else {
    abort = true;
  }
...
void showErrorScreen(const char* message) {
  ...
  renderer.drawCenteredText(UI_FONT_ID, 100, message, true);
...
bool earlyInit() {
  ...
  const bool skipWakeVerification =
      papyrix::core.settings.pendingTransition == 2 || papyrix::hasSleepResumeTransition();
  if (wakeup.isPowerButton && !skipWakeVerification) {
    verifyWakeupLongPress(wakeup.resetReason);
  } else if (wakeup.isPowerButton && skipWakeVerification) {
    LOG_INF(TAG, "Skipping wakeup verification for reader sleep-resume");
  }
...
void initUIMode() {
  ...
  LOG_DBG(TAG, "[UI mode] Free heap: %u, Max block: %u", ESP.getFreeHeap(), ESP.getMaxAllocHeap());

  // UI mode should always run at full CPU speed even if the previous reader
  // session throttled before triggering a restart back to UI.
  ...
  LOG_DBG(TAG, "[UI mode] After init - Free heap: %u, Max block: %u", ESP.getFreeHeap(), ESP.getMaxAllocHeap());
}

// Initialize Reader mode - minimal state registration, single font size
void initReaderMode() {
  LOG_INF(TAG, "Initializing READER mode");
  LOG_DBG(TAG, "[READER mode] Free heap: %u, Max block: %u", ESP.getFreeHeap(), ESP.getMaxAllocHeap());

  const auto& transition = papyrix::getTransition();

  // Early check: no book path means nothing to read — fall back to UI before any setup
  ...
  if (papyrix::hasSleepResumeTransition()) {
    LOG_INF(TAG, "Skipping initial reader render after sleep-resume");
  } else {
    // Force initial render
    LOG_DBG(TAG, "Forcing initial render");
    stateMachine.update(papyrix::core);
  ...
  LOG_DBG(TAG, "[READER mode] After init - Free heap: %u, Max block: %u", ESP.getFreeHeap(), ESP.getMaxAllocHeap());
}
...
void loop() {
  ...
  if (!papyrix::core.cpu.isThrottled() && millis() - lastMemPrint >= 10000) {
    LOG_DBG(TAG, "Free: %u bytes, Total: %u bytes, Min Free: %u bytes, MaxAlloc: %u bytes", ESP.getFreeHeap(),
  ...
  const bool readerActive = stateMachine.isInState(papyrix::StateId::Reader);
  papyrix::core.tilt.poll(papyrix::core.settings.orientation, readerActive && papyrix::core.settings.tiltPageTurn != 0);
  if (papyrix::core.tilt.hadActivity()) {
  ...
  if (autoSleepTimeout > 0 && papyrix::core.input.idleTimeMs() >= autoSleepTimeout) {
    LOG_INF(TAG, "Auto-sleep after %u ms idle", autoSleepTimeout);
