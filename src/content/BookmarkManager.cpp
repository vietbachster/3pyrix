...
bool BookmarkManager::save(Core& core, const char* cacheDir, ContentType type, const Bookmark* bookmarks, int count) {
  ...
  char path[280];
  int pathLen = snprintf(path, sizeof(path), "%s/bookmarks.bin", cacheDir);
  if (pathLen < 0 || static_cast<size_t>(pathLen) >= sizeof(path)) {
    LOG_ERR(TAG, "Cache dir path too long");
    return false;
  }
  ...
  bool writeOk = (file.write(&cnt, 1) == 1);
  if (count > 0 && writeOk) {
    const size_t dataSize = static_cast<size_t>(count) * sizeof(Bookmark);
    writeOk = (file.write(reinterpret_cast<const uint8_t*>(bookmarks), dataSize) == dataSize);
  }
  file.close();
  if (!writeOk) {
    LOG_ERR(TAG, "Write error while saving bookmarks (disk full?)");
    return false;
  }
  LOG_DBG(TAG, "Saved %d bookmarks", count);

  pathLen = snprintf(path, sizeof(path), "%s/bookmarks.txt", cacheDir);
  if (pathLen < 0 || static_cast<size_t>(pathLen) >= sizeof(path)) {
    LOG_ERR(TAG, "Cache dir path too long for txt");
    return true;  // .bin was saved successfully
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (len > 0) {
      if (file.write(reinterpret_cast<const uint8_t*>(line), static_cast<size_t>(len)) != static_cast<size_t>(len)) {
        LOG_ERR(TAG, "Write error on bookmark %d (disk full?)", i);
        break;
      }
    }
  }
