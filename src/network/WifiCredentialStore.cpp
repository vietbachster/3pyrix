...
bool WifiCredentialStore::saveToFile() const {
  ...
  for (int i = 0; i < count_ && ok; i++) {
    ...
    ok &= (file.write(reinterpret_cast<const uint8_t*>(obfuscated), pwdLen) == pwdLen);
  }

  file.close();

  if (!ok) {
    LOG_ERR(TAG, "Write error while saving credentials (disk full?)");
    return false;
  }
...
void WifiCredentialStore::clearAll() {
  count_ = 0;
  if (!saveToFile()) {
    LOG_ERR(TAG, "Failed to persist credential clear to disk");
  }
