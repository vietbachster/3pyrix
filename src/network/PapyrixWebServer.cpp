...
void PapyrixWebServer::handleStatus() {
  ...
  int n = snprintf(json, sizeof(json),
                   ...
                   PAPYRIX_VERSION, ipAddr.c_str(), apMode_ ? "AP" : "STA", apMode_ ? 0 : WiFi.RSSI(),
                   ESP.getFreeHeap(), millis() / 1000);

  if (n < 0 || static_cast<size_t>(n) >= sizeof(json)) {
    server_->send(500, "application/json", "{\"error\":\"status too long\"}");
    return;
  }
...
void PapyrixWebServer::handleFileListData() {
  ...
  while (file) {
    ...
    if (name[0] != '.' && !FsHelpers::isHiddenFsItem(name)) {
      ...
      } else if (written >= sizeof(output)) {
        LOG_ERR(TAG, "File entry JSON truncated for '%s', skipping", name);
      }
    }
...
void PapyrixWebServer::handleCreateFolder() {
  ...
  if (folderName.indexOf("..") >= 0 || folderName.indexOf('/') >= 0) {
    server_->send(400, "text/plain", "Invalid folder name");
    return;
  }
